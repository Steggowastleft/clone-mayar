...
import { ProdukDigitalData, AnalisisData } from "../show";
...
export default function TabAnalisis({ produk, analisis }: Props) {
  ...
  const growthSign = analisis.growth_percent >= 0 ? "+" : "";
  const growthColor = analisis.growth_percent >= 0 ? "text-emerald-600" : "text-rose-600";

  // Conversion rate from checkout to sukses
  const conversionRate = analisis.checkout_count > 0
    ? Math.round((analisis.transaksi_sukses / analisis.checkout_count) * 100)
    : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-200/50 rounded-2xl p-5 flex items-start gap-4">
        ...
        <div>
          ...
          {analisis.total_transaksi > 0 ? (
            <p className="text-sm text-gray-600 mt-0.5">
              Produk <strong className="text-purple-700 font-semibold">{produk.nama}</strong> memiliki{" "}
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Belum ada transaksi untuk produk <strong>{produk.nama}</strong>. Data analisis akan muncul setelah ada transaksi masuk.
            </p>
          )}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <h4 className="text-2xl font-bold text-gray-800 mt-1">{analisis.total_transaksi}</h4>
              <p className="text-xs text-gray-400 mt-0.5">
                {analisis.transaksi_sukses} sukses · {analisis.transaksi_gagal} gagal
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <h4 className="text-2xl font-extrabold text-gray-800 mt-1">{formatRupiahFull(analisis.nominal_transaksi)}</h4>
              {analisis.growth_percent !== 0 && (
                <p className={`text-xs font-semibold mt-0.5 ${growthColor}`}>
                  {growthSign}{analisis.growth_percent}% vs minggu lalu
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <h4 className="text-2xl font-bold text-gray-800 mt-1">{analisis.transaksi_pending}</h4>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {/* Funnel 1: Page Views (checkout_count as proxy) */}
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col justify-between">
            ...
            <div className="mt-4">
              ...
              <h4 className="text-2xl font-extrabold text-gray-800 mt-1">{analisis.checkout_count}</h4>
              <p className="text-xs text-slate-500 mt-1">Peserta yang mendaftar ke produk ini</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col justify-between">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-amber-500">
                {analisis.total_transaksi > 0 ? `${Math.round((analisis.total_transaksi / Math.max(analisis.checkout_count, 1)) * 100)}%` : "0%"} Checkout
              </span>
            </div>
            <div className="mt-4">
              <p className="text-xs text-gray-400 font-medium">Selesai Checkout</p>
              <h4 className="text-2xl font-extrabold text-gray-800 mt-1">{analisis.total_transaksi}</h4>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col justify-between">
            ...
            <div className="mt-4">
              ...
              <h4 className="text-2xl font-extrabold text-gray-800 mt-1">{analisis.transaksi_sukses}</h4>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        {analisis.chart_data.length > 0 && analisis.total_transaksi > 0 ? (
