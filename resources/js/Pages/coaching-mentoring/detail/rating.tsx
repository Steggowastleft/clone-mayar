...
import { id } from "date-fns/locale"; // ✅ FIX DISINI
...
interface Props {
  coachingId: number; // ✅ TAMBAHIN INI
  ratings: RatingItem[];
}
...
function StarDisplay({ value, size = "md" }: { value: number; size?: "sm" | "md" | "lg" }) {
  const sz = size === "sm" ? "h-3 w-3" : size === "lg" ? "h-5 w-5" : "h-4 w-4";
  return (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-xs text-gray-400 shrink-0">
              {format(new Date(rating.created_at), "d MMM yyyy", { locale: id })} {/* ✅ FIX */}
            </span>
          </div>
...
export default function TabRating({ ratings }: Props) {
  ...
  const dist = [5, 4, 3, 2, 1].map((b) => ({
    ...
    pct:
      total > 0
        ? Math.round(
            (ratings.filter((r) => r.bintang === b).length / total) * 100
          )
        : 0,
  }));
  ...
  const filtered = ratings.filter((r) => {
    const matchBintang =
      filterBintang === null || r.bintang === filterBintang;

    const nama = r.tampil_anonim
      ? "anonim"
      : r.nama_peserta.toLowerCase();
