      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className={cn(
              "w-full flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-md bg-white text-sm text-left hover:border-gray-300 transition focus:outline-none focus:ring-2 focus:ring-purple-500",
...
export function EditProdukDigitalDialog({
  ...
  const [formData, setFormData] = useState({
    ...
    jumlahHalaman: "",
    bisaDidownload: true,
    tipeTulisan: "one_shot" as "one_shot" | "chapter" | "tamat" | "ongoing",
  ...
  // Sync data from produk when loaded
  useEffect(() => {
    if (produk) {
      setFormData({
        nama: produk.nama || "",
        deskripsi: produk.deskripsi || "",
        kategori: (produk.kategori === "template" ? "" : produk.kategori) || "",
        tipePembayaran: produk.tipe_pembayaran || "gratis",
        harga: produk.harga !== undefined && produk.harga !== null ? String(produk.harga) : "",
        hargaCoret: produk.harga_coret !== undefined && produk.harga_coret !== null ? String(produk.harga_coret) : "",
        sumberFile: produk.sumber_file || "upload",
        redirectUrl: produk.sumber_file === "file_lama" ? (produk.file_lama_id || "") : (produk.redirect_url || ""),
        catatan: produk.catatan || "",
        maxPembayaran: produk.max_pembayaran !== undefined && produk.max_pembayaran !== null ? String(produk.max_pembayaran) : "",
        author: produk.author || "",
        isbn: produk.isbn || "",
        format: produk.format || "PDF",
        bahasa: produk.bahasa || "Indonesia",
        jumlahHalaman: produk.jumlah_halaman !== undefined && produk.jumlah_halaman !== null ? String(produk.jumlah_halaman) : "",
        bisaDidownload: produk.bisa_didownload !== undefined && produk.bisa_didownload !== null ? !!produk.bisa_didownload : true,
        tipeTulisan: (produk.tipe_tulisan || "one_shot") as any,
        mekanismeBayar: (produk.mekanisme_bayar || "sekali_bayar") as any,
        genre: produk.genre || "",
        transkrip: produk.transkrip || "",
        pembicara: produk.pembicara || "",
        durasi: produk.durasi || "",
        artis: produk.artis || "",
        kategori_produk: produk.kategori_produk || "",
        tipe_pembaca: produk.tipe_pembaca || "Gulir Vertikal",
      });

      setWaktuMulaiJual(produk.waktu_mulai_jual_raw ? new Date(produk.waktu_mulai_jual_raw) : undefined);
      setTanggalKadaluarsa(produk.tanggal_kadaluarsa_raw ? new Date(produk.tanggal_kadaluarsa_raw) : undefined);
      setTanggalPublish(produk.tanggal_publish_raw ? new Date(produk.tanggal_publish_raw) : undefined);
  ...
  const handleSave = () => {
    ...
    router.post(`/produk-digital/${produk.id}`, payload, {
      forceFormData: true,
      onSuccess: () => {
      ...
      onError: (errors) => {
        setIsSubmitting(false);
        const errorMsg = Object.values(errors)[0] as string || "Terjadi kesalahan saat memperbarui produk";
      ...
      <DialogContent aria-describedby={undefined} className="max-w-2xl max-h-[90vh] overflow-y-auto p-0">
        <DialogHeader className={cn("p-6 text-white rounded-t-lg", theme.bgHeader)}>
          <DialogTitle className="text-xl font-bold flex items-center gap-2">
            ...
            Edit Produk Digital: {theme.name}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="p-6 space-y-5">
          ...
          {formData.kategori === "" ? (
            ...
            <>
              ...
              {/* Cover Produk */}
              <div className="space-y-1">
                <Label className="text-sm font-medium text-gray-700">Cover Produk</Label>
                <div
                ...
                >
                  {coverPreview ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-gray-500 font-semibold text-blue-600">Pilih Gambar...</p>
                      <p className="text-xs text-gray-400">Drag & drop gambar cover di sini</p>
                    </div>
                  )}
                </div>
                <input
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <Button
                                ...
                                onClick={() => document.getElementById('edit-bulk-upload-pages-all')?.click()}
                              >
                                <Upload className="h-3.5 w-3.5 text-gray-500" /> Upload Sekaligus
                              </Button>
                              <input
                                id="edit-bulk-upload-pages-all"
                                type="file"
                                multiple
