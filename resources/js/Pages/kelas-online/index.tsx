...
export default function KelasOnline({ produk = [] }: Props) {
  ...
  return (
    <DashboardLayout>
      <Head title="Kelas Online (OD)" />
      <div className="flex gap-0 min-h-screen">
        <div className="flex-1 p-6">
          <p className="text-xs text-gray-400 uppercase tracking-wide mb-1">PROJEK</p>
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold text-gray-800">Kelas Online (OD)</h1>
            <div className="flex gap-2">
              <Button variant="outline" className="border-blue-500 text-blue-600 hover:bg-blue-50" onClick={() => window.open("/kelas-online/catalog", "_blank")}>
                PRODUK
              </Button>
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-semibold text-gray-700">Semua Kelas Online (OD)</h2>
                  ...
                  {filtered.map((p) => (
                    <div key={p.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Button size="sm" onClick={() => router.visit(`/kelas-online/${p.id}`)}>Detail</Button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="w-72 border-l border-gray-200 bg-gray-50 p-4 space-y-3 shrink-0">
          <Input
            placeholder="Cari Kelas Online (OD)"
          ...
          <button
            onClick={() => window.open("/kelas-online/catalog", "_blank")}
          ...
          <p className="text-xs text-gray-500 text-center leading-relaxed">
            Katalog adalah halaman online dimana semua Kelas Online (OD) Anda yang aktif ditampilkan.
          </p>
          <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold">
            + Buat Kelas Online (OD) Baru
          </Button>
        </div>
      </div>
    </DashboardLayout>
