...
import { Download, Printer, ExternalLink, ShoppingBag } from "lucide-react";
...
type Props = { produk: ProdukDigital[] };

export default function ProdukDigitalIndex({ produk = [] }: Props) {
    ...
    <DashboardLayout>
      ...
      <div className="flex gap-0 min-h-screen">
                  ...
                  {filtered.map((p) => (
                    <div key={p.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Button size="sm" onClick={() => router.visit(`/produk-digital/${p.id}`)}>Detail</Button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="w-72 border-l border-gray-200 bg-gray-50 p-4 space-y-3 shrink-0">
          <Input placeholder="Cari Produk Digital" className="bg-white text-sm"
            value={search} onChange={(e) => setSearch(e.target.value)} />
          ...
          <button onClick={() => window.open("/produk-digital/catalog", "_blank")}
