      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-dashed divide-slate-200">
            {data.length === 0 ? (
              <tr>
                <td colSpan={8} className="text-center text-slate-455 py-16 text-sm">
                  {emptyMsg}
                </td>
              </tr>
            ) : (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <Popover open={dateOpenPaid} onOpenChange={setDateOpenPaid}>
                <PopoverTrigger asChild>
                  <button
                    className={cn(
                      "px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-500 hover:bg-slate-50 flex items-center gap-1.5 h-9 min-w-32 justify-between"
                    )}
                  >
                    <span className="truncate">
                      {dateFilterPaid ? format(dateFilterPaid, "dd/MM/yyyy") : "Pilih Tanggal"}
                    </span>
        ...
        {/* Table Body */}
        {renderTable(filteredPaid, "Belum ada transaksi pembayaran.")}
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-slate-100 space-y-3 bg-white">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-extrabold text-slate-800 uppercase tracking-wider">
              Semua Pelanggan Belum Bayar Projek Ini
            </h3>
            <span className="text-[10px] font-extrabold px-2.5 py-0.5 rounded-full bg-yellow-50 text-yellow-750 border border-yellow-150">
              {filteredUnpaid.length} Orang
            </span>
          </div>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            {/* Search */}
            ...
            <Popover open={dateOpenUnpaid} onOpenChange={setDateOpenUnpaid}>
              <PopoverTrigger asChild>
                ...
                >
                  <span className="truncate">
                    {dateFilterUnpaid ? format(dateFilterUnpaid, "dd/MM/yyyy") : "Pilih Tanggal"}
                  </span>
                  <CalendarIcon className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                </button>
              </PopoverTrigger>
        ...
        {/* Table Body */}
        {renderTable(filteredUnpaid, "Belum ada tagihan pending.")}
      </div>
    </div>
  );
}