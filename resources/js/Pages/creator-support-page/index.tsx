...
export default function CreatorSupportPage({ produk = [] }: Props) {
  ...
  return (
    <DashboardLayout>
      <Head title="Creator Support Page" />
      <div className="flex gap-0 min-h-screen">
        <div className="flex-1 p-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-semibold text-gray-700">Semua Creator Support Page</h2>
                  ...
                  {filtered.map((p) => (
                    <div key={p.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Button size="sm" onClick={() => router.visit(`/creator-support-page/${p.id}`)}>Detail</Button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="w-72 border-l border-gray-200 bg-gray-50 p-4 space-y-3 shrink-0">
          <Input
            placeholder="Cari Creator Support Page"
          ...
          <button
            onClick={() => window.open("/creator-support-page/catalog", "_blank")}
          ...
          >
            KATALOG CREATOR SUPPORT
            <ExternalLink className="h-4 w-4" />
          </button>
          <p className="text-xs text-gray-500 text-center leading-relaxed">
            Katalog adalah halaman online dimana semua Creator Support Page Anda yang aktif ditampilkan.
          </p>
          <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold">
            + Buat Creator Support Page Baru
          </Button>
        </div>
      </div>
    </DashboardLayout>
