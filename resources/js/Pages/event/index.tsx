...
import { Download, Printer, ExternalLink, Calendar } from "lucide-react";
...
export default function EventIndex({ events = [] }: Props) {
  ...
  const filtered = events.filter((e) => {
    const matchSearch = e.nama.toLowerCase().includes(search.toLowerCase());
    ...
    <DashboardLayout>
      ...
      <div className="flex gap-0 min-h-screen">
        <div className="flex-1 p-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-semibold text-gray-700">Semua Event</h2>
            ...
            <div className="p-6">
              {filtered.length === 0 ? (
                ...
                <div className="space-y-3">
                  {filtered.map((e) => (
                    <div key={e.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition">
                      <div>
                        <p className="font-semibold text-gray-800">{e.nama}</p>
                        <p className="text-sm text-gray-500">{e.lokasi} · {e.tanggal} · {e.peserta} peserta</p>
                      </div>
                      <div className="flex items-center gap-3">
                        {statusBadge(e.status)}
                        <Button size="sm" onClick={() => router.visit(`/event/${e.id}`)}>Detail</Button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="w-72 border-l border-gray-200 bg-gray-50 p-4 space-y-3 shrink-0">
          <Input placeholder="Cari Event" className="bg-white text-sm"
            value={search} onChange={(e) => setSearch(e.target.value)} />
          ...
          <button onClick={() => window.open("/event/catalog", "_blank")}
            ...
            KATALOG EVENT <ExternalLink className="h-4 w-4" />
          </button>
          <p className="text-xs text-gray-500 text-center leading-relaxed">
            Halaman katalog online untuk semua Event Anda yang aktif.
          </p>
