...
import { Download, Printer, ExternalLink, Package } from "lucide-react";
...
type Props = { bundles: Bundle[] };

export default function BundleIndex({ bundles = [] }: Props) {
    ...
    <DashboardLayout>
      ...
      <div className="flex gap-0 min-h-screen">
        <div className="flex-1 p-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-semibold text-gray-700">Semua Bundle</h2>
                  ...
                  {filtered.map((b) => (
                    <div key={b.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{b.isi_produk} produk · {b.tanggal} · {b.terjual} terjual</p>
                      </div>
                      <div className="flex items-center gap-3">
                        {statusBadge(b.status)}
                        <Button size="sm" onClick={() => router.visit(`/bundle/${b.id}`)}>Detail</Button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="w-72 border-l border-gray-200 bg-gray-50 p-4 space-y-3 shrink-0">
          <Input placeholder="Cari Bundle" className="bg-white text-sm"
            value={search} onChange={(e) => setSearch(e.target.value)} />
          ...
          <button onClick={() => window.open("/bundle/catalog", "_blank")}
            ...
            KATALOG BUNDLE <ExternalLink className="h-4 w-4" />
          </button>
          <p className="text-xs text-gray-500 text-center leading-relaxed">
            Halaman katalog online untuk semua Bundle Anda yang aktif.
          </p>
