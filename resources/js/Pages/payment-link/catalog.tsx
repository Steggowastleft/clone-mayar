...
export default function KatalogPaymentLink({ produk }: Props) {
  const list = produk ?? [];

  return (
    <>
      <Head title="Katalog Link Pembayaran" />

      <div className="min-h-screen bg-gray-50">

        {/* HEADER */}
        <div className="bg-gradient-to-r from-slate-900 to-sky-900 text-white py-12">
          <div className="max-w-6xl mx-auto px-4">
            <h1 className="text-3xl font-extrabold">Katalog Link Pembayaran</h1>
            <p className="text-slate-300 mt-2 text-sm">
              {list.length} link pembayaran tersedia — mudah dan cepat 🚀
            </p>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 py-8 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {list.length === 0 && (
            <p className="col-span-full text-center text-gray-400 py-16">
              Belum ada link pembayaran yang tersedia.
            </p>
          )}
                      ...
                      onClick={() =>
                        cfg
                          ? router.visit(
                              p.type === "bootcamp" 
                                ? cfg.route 
                                : `${cfg.route}/${p.product_id}/p`
                            )
                          : undefined
                      }
                      className="text-xs bg-sky-600 hover:bg-sky-700 text-white px-3 py-2 rounded-md flex items-center gap-1"
                    >
